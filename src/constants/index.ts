...
export const SAJU_RESULT_ENDING = {
  title: '카드 해석을 마치며...',
  endingContent: `
    탄생 카드는 태어날 때부터 가지고 있는 기질과 성향을 보여주는 나침반입니다.
    카드가 1장, 2장 혹은 3장이 나온 분들도 계실 텐데요.
    카드 개수 보다는 각각의 카드가 가지고 있는 의미와 상징을 이해하고, 나의 삶에 어떻게 적용할 수 있는지 고민해 보는 것이 중요합니다.
    
    카드를 많이 쓰는 것이 아니라 상황에 따라 다른 카드를 꺼내 쓸 수 있는 훈련을 해보세요.
    예를 들어 일할 때는 이 카드의 기질을, 집 안에서는 이 카드를, 친한 사람들과 있을 때는 이 카드를, 낯선 사람에게는 이 카드를 쓰는 식으로 말이죠.
